...
export default function SupportSellerScreen() {
  ...
  const [messages, setMessages] = useState<any[]>([
    ...
    {
      id: '2',
      text: "A customer didn't pick up their order.",
      senderId: user?.uid || 'user1',
      createdAt: { seconds: Date.now() / 1000 - 30 }
    },
    {
      id: '3',
      text: "I'm sorry about that.\nI can help you cancel the order or contact the buyer.\nWhich would you prefer?",
      senderId: 'bot',
        ...
        <FlatList
          ...
          ListFooterComponent={() => (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: scale(8), marginTop: verticalScale(8) }}>
              <Pressable onPress={() => handleQuickReply('Cancel order')} style={{ paddingHorizontal: scale(12), paddingVertical: verticalScale(10), borderRadius: scale(12) }} className="border border-gray-200 flex-row items-center bg-white active:bg-gray-50">
                <RefreshCw size={scale(16)} color="#1B7A49" />
                <Text style={{ fontSize: moderateScale(13), marginLeft: scale(8) }} className="font-medium text-gray-700">Cancel order</Text>
              </Pressable>
              ...
              <Pressable onPress={() => handleQuickReply('Payout help')} style={{ paddingHorizontal: scale(12), paddingVertical: verticalScale(10), borderRadius: scale(12) }} className="border border-gray-200 flex-row items-center bg-white active:bg-gray-50">
                <CreditCard size={scale(16)} color="#1B7A49" />
                <Text style={{ fontSize: moderateScale(13), marginLeft: scale(8) }} className="font-medium text-gray-700">Payout help</Text>
              </Pressable>
